...
### 1) Step 1 요청: 맵 이미지 로더/파서 구현
반영 여부: 그대로 반영
이유: 파일명은 `MapLoader.cs` 하나로 요청했지만, 이전 세션(01번)에서 이미 정해둔 폴더별 역할 분리 원칙에 따라 자연스럽게 `MapImageLoader`/`MapImageParser`/`OccupancyGrid` 세 파일로 구현된 것이라 별도로 수정을 요청할 부분은 없었음.

### 2) 코드 품질 검토 요청 (예외 처리 / 병목 / 메모리 누수)
반영 여부: 그대로 반영
이유: 문제가 있는지 물어봤을 때 발견되면 바로 고쳐주길 바랐고, 그렇게 진행됨.

### 3) MapImageLoader 크기 값을 파라미터로 분리 요청
반영 여부: 그대로 반영
이유: 이미지 크기(1024x768)를 하드코딩하지 말고 파라미터로 빼달라고 했고, 그대로 반영됨.

### 4) Step 2 진행 요청
반영 여부: 그대로 반영
이유: 다음 단계로 이어서 진행해달라는 요청이라 별도 수정 없이 반영됨.
