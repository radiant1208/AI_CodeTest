...
## 사용자 피드백
- 파라미터(Footprint 크기, 안전마진 등)를 대화 중간에 직접 파일을 수정해 가며 반복적으로 실험하는 방식을 선호 — Claude가 그 변경분을 인지하고 이어서 진단/설명해주길 기대함.
- 검증/디버그 목적의 코드는 `#if DEBUG`로 감싸 Release 빌드에 영향이 없도록 하는 패턴을 명시적으로 요구함 → 이후 디버그 전용 코드는 이 패턴을 기본으로 따름.
- 성능/메모리(GC) 관련 검증을 요청할 때는 ms, KB 단위의 실측 수치로 된 근거 제시를 기대함.

## 참고
- 이 대화에서 다룬 `ObstacleInflator` 구현 자체(Step 2 최초 작성)는 `prompts/02_map_parsing.md`에 기록되어 있으며, 본 문서는 그 이후의 디버그 검증 테스트 구축 및 파라미터 튜닝 과정만을 다룸.
- `RunObstacleInflatorDebugTest`는 `src/Program.cs`에 영구적으로 남아있으며(`#if DEBUG`), Debug 빌드로 `dotnet run` 시마다 자동 실행됨.
