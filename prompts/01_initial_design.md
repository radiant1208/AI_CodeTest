...
## 사용자 피드백
- 매 단계 제시된 구조/결과물을 승인하고 다음 단계 진행을 지시함 (반려/수정 요청 없음).
- 프롬프트 기록을 스텝 단위로 계속 최신화해 나가는 방식을 선호함 → 새 번호 파일을 매번 만들기보다, 관련된 설계/스캐폴딩 흐름은 하나의 `01_initial_design.md`에 이어서 누적 기록.
- 코드에 남는 주석은 장황한 설명(사용처, 계산 방식)보다 "핵심 개념 + 단위"만 담은 한 줄 인라인 스타일을 선호함.
- 설정값 접근용으로 커스텀 POCO+수동 파서를 새로 만들기보다, 이미 표준화된 프레임워크 기능(`Microsoft.Extensions.Configuration`)을 정적 접근 형태로 활용하는 방식을 선호함.

## 참고
- 확정된 아키텍처 전체 내용은 `.claude/CLAUDE.md`에 프로젝트 컨텍스트로 별도 저장·유지됨 (내용 변경 시 이 파일도 함께 갱신).
