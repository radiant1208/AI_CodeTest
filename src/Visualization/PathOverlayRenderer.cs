...
{
    /// <summary>경로(HybridState 목록)를 이미지 위에 그리는 순수 렌더링 로직(파일 I/O 없음).
    /// 차량 Footprint는 FE 애니메이션이 담당하므로, 여기서는 다운로드용 정적 이미지에 필요한
    /// "원본 이미지 + 경로"만 그린다(차량 표시 없음).</summary>
    public static class PathOverlayRenderer
    {
        ...
        /// <summary>원본 이미지를 복제해 경로 세그먼트(전진=주황/후진=빨강)와 각 노드의 heading 틱(하늘색)을 그린
        /// 새 Mat을 반환한다.</summary>
